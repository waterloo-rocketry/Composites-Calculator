...
##len(plystress) = len(maxstresses)

#plystress = np.zeros(shape=(3,len(height)))
#maxstresses = np.ones(shape=(5,len(height)))
...
def failure_criterion(plystress, maxstress):
    has_not_failed = True
    ply_failure_indicies = []
    ply_failure_recripricol_tsai_wu_values = []
    ...
    for i in range(len(plystress[0])):
        ...
        if (tsai_wu_value >= 1):
            ply_failure_indicies.append(i)
            has_not_failed = False
            ply_failure_recripricol_tsai_wu_values.append(1/tsai_wu_value)
